...
def talker():
    ...
    pub = rospy.Publisher('bbox_position', multipositions, queue_size=10)
    #pub1 = rospy.Publisher('bbox_position', multipositions, queue_size=10)
    rospy.init_node('bbox_pub', anonymous=False)
    ...
    while not rospy.is_shutdown():
        ...
        '''
        float_array = multipositions()
        for i in range(len(ObjectsList)):
            float_list = position()
            float_list.elements = ObjectsList[i]
