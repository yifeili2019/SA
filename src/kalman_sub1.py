...
def fn_EKF_item_M1(item_predict,measurement):
    ...
    #X_s = F*X_origin + delt_time*V_origin
    P_s = np.dot(F,np.dot(P_init,F.T)) + Q
    print("P_S:",P_s)
    y_k = measure_next - X_s
    print("y_k:",y_k)

    S = np.dot(H,np.dot(P_s,H.T)) + R
    print("s:",S)
    k = np.dot(P_s,np.dot(H.T,(inv(S))))
    print("K:",k)
    X = X_s + np.dot(k,y_k)
    print("x:",X)
    P = np.dot((np.identity(3)-np.dot(k,H)),P_s)
    print("p:",P)

    P_init = P
    return X,measurement[3:length]
...
def callback(data):
    ...
    if(j>=3):                                                    #kalman filter should start from frame3
        ...
        for i in range(len(data.lists)):
            ...
            temp5 = data.lists[i].dx
            temp6 = data.lists[i].dy
            temp7 = data.lists[i].dz
            list_tmp.append(temp1)
            list_tmp.append(temp2)
            list_tmp.append(temp3)
            list_tmp.append(temp4)
            list_tmp.append(temp5)
            list_tmp.append(temp6)
            list_tmp.append(temp7)
        ...
        create_file.write("frame_3_measure")
        create_file.write(str(newlist_frame_3))
        create_file.write("\n")
        #find optimal result of matched item
        optimal_frame_3,uncertainty_list = fn_EKF_list_M1(newlist_prediction,newlist_frame_3)
        create_file.write("update:")
        create_file.write(str(optimal_frame_3))
        create_file.write("\n")
        ...
        for i in range(len(list3_left)):                          #put item left by frame3 in pub_data
            ...
            pos_temp.dx = list3_left[i][4]
            pos_temp.dy = list3_left[i][5]
            pos_temp.dz = list3_left[i][6]
        ...
        for i in range(len(optimal_frame_3)):                      #put optimal items in pub_data
            ...
            pos_temp.uncertainty = uncertainty_list[i][0]
            pos_temp.dx = uncertainty_list[i][1]
            pos_temp.dy = uncertainty_list[i][2]
            pos_temp.dz = uncertainty_list[i][3]
    ...
    if(j>=3):
        ...
        for i in range(len(optimal_frame_3)):                      #put optimal items in pub_data
            ...
            pos_temp.append(uncertainty_list[i][0])
            pos_temp.append(uncertainty_list[i][1])
            pos_temp.append(uncertainty_list[i][2])
            pos_temp.append(uncertainty_list[i][3])
    ...
    create_file.write("velocity:")
    create_file.write(str(velocity_list))
    create_file.write("\n")
    ...
    create_file.write("predictPositions")
    create_file.write(str(predictPositions))
    create_file.write("\n")
    ...
    if(j<3):
        ...
        for i in range(len(data.lists)):
            ...
            temp.p = 100
            temp.uncertainty = data.lists[i].uncertainty
            temp.dx = data.lists[i].dx
            temp.dy = data.lists[i].dy
            temp.dz = data.lists[i].dz
        ...
        #create_file.write(str(k))
        #create_file.write(str(pub_data))
        #create_file.write("\n")
        #k = k+1
...
if __name__ == '__main__':
    #EKF parameters
    create_file = open("/home/dell/test1/src/gdp/src/kalman.txt","w")
    F = np.identity(3)
    H = np.identity(3)
